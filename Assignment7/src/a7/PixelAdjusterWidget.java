...
/*
 * ********************************************************************************************************
 THIS IS ALL SCRAPED CODE
 BLUR
//for the first pixel (0, 0)
//you get the area of blurness(1)
// (-1, -1) --> (1,1)
//if corner is out of bounds
//set to (0,0) or (width-1, height-1)
//for loop through 
//totRed-=p.getPixel(i, j).getRed();
//totGre-=p.getPixel(i, j).getGreen();
//totBlu-=p.getPixel(i, j).getBlue();

//System.out.println("---------");
//avgRed=totRed/(count-1);
//avgGre=totGre/(count-1);
//avgBlu=totBlu/(count-1);
//Pixel blurPix=new ColorPixel(avgRed, avgGre, avgBlu);
//copy.setPixel(i, j, blurPix);
/*				if(i==0&&j==0){
System.out.println(top_x+"  "+top_y+"  "+ bottom_x+"  "+bottom_y);
}
if(top_x<0){
top_x=0;
}
...
if (bottom_x>=p.getWidth()){
bottom_x=p.getWidth()-1;
}
if(bottom_y>=p.getHeight()){
bottom_y=p.getHeight()-1;
}
Math.max(top_x, 0);
math.m
System.out.println(top_x+"  "+top_y+"  "+ bottom_x+"  "+bottom_y+"  "+p.getHeight());

if my coordinate is (0,0)
0,0 
STATE CHANGED METHOD
		...
		//copy=blurPix(copy, blurSlide.getValue());
		//blurSlide.get
